const Store = {
    ...
    async addOrder(orderData) {
        try {
            ...
            const docRef = await addDoc(collection(this.db, "orders"), finalData);
    ...
    async addProduct(productData) {
        try {
            const { collection, addDoc } = window.firebase;
            // Get max ID for simple increment
            const maxId = this.products.length > 0 ? Math.max(...this.products.map(p => p.id || 0)) : 0;
